...
// // 🔴 Recording bar with glassmorphism + pulsing dot + transition
...
//   recordingBar.style.bottom = "-100px"; // hidden initially
...
//   if (recordingBar) {
//     recordingBar.style.bottom = "-100px"; // slide out
//     setTimeout(() => {
//       if (recordingBar) {
//         recordingBar.remove();
//         recordingBar = null;
...
// // 🟢 Saved popup with glassmorphism
...
//     startRecording("both", true); // manual = pulsing dot + stop button
//   }
...
//       const mode = data.recordMode || "both";
...
async function startRecording(mode, isManual = false) {
  ...
  try {
    ...
    if (mode === "audio") {
      ...
      // Screen only (will ask)
      stream = await navigator.mediaDevices.getDisplayMedia({ video: true });
    } else if (mode === "screenaudio") {
      // Screen + Audio (will ask)
      stream = await navigator.mediaDevices.getDisplayMedia({ video: true, audio: true });
    }
...
chrome.runtime.onMessage.addListener((msg) => {
  if (msg.type === "manual-start") {
    startRecording("cameraaudio", true); // Manual = webcam + mic + stop button
  }
  if (msg.type === "manual-stop") {
    stopRecording();
  }
