...
function setupEndButtonDetection() {
  ...
  // ============================================================
  // ROBUST BUTTON DETECTION - WORKS ON ALL ZOOM VERSIONS
  // ============================================================
  function attachLeaveButtonListener() {
    ...
    // Strategy 2: If selectors fail, search for buttons by text content
    if (!leaveBtn || !endForAllBtn) {
      ...
      allButtons.forEach(btn => {
        ...
        if (!leaveBtn && text === 'leave meeting') {
          leaveBtn = btn;
        }
        if (!endForAllBtn && text === 'end meeting for all') {
          endForAllBtn = btn;
        }
      });
    }
    ...
    if (leaveBtn && !leaveBtn.hasAttribute('data-leave-listener')) {
      ...
      leaveBtn.addEventListener('click', () => {
        console.log("🎯🎯🎯 LEAVE MEETING BUTTON CLICKED - STOPPING RECORDING 🎯🎯🎯");
        stopRecording();
      });
    ...
    if (endForAllBtn && !endForAllBtn.hasAttribute('data-end-listener')) {
      ...
      endForAllBtn.addEventListener('click', () => {
        console.log("🎯🎯🎯 END MEETING FOR ALL BUTTON CLICKED - STOPPING RECORDING 🎯🎯🎯");
        stopRecording();
      });
  ...
  // ============================================================
  // MUTATION OBSERVER - AUTO-DETECT NEW BUTTONS
  // ============================================================
  function setupMutationObserver() {
    const observer = new MutationObserver(() => {
      // Every time DOM changes, try to attach listeners
    ...
    console.log("✅ MutationObserver active - will auto-detect buttons when they appear");
  }
  
  // Attach listener immediately
  attachLeaveButtonListener();
  
  // Setup automatic detection for when buttons appear
  setupMutationObserver();
  
  // Also retry every 1 second as backup
  setInterval(() => {
    attachLeaveButtonListener();
  }, 1000);
