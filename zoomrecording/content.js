...
// DIAGNOSTIC: Log EVERY click to see if clicks reach extension
let totalClicks = 0;
window.addEventListener('click', (e) => {
  totalClicks++;
  const target = e.target;
  const text = (target.textContent || '').toLowerCase().trim().substring(0, 40);
  const aria = (target.getAttribute('aria-label') || '').toLowerCase();
  
  // Log EVERY single click to console
  if (totalClicks <= 100) { // Limit to first 100 to avoid spam
    console.log(`[CLICK #${totalClicks}] tag=${target.tagName}, text="${text}", aria="${aria}"`);
  }
  
  // Specifically check for leave/end keywords
  if (text.includes('leave') || text.includes('end') || aria.includes('leave') || aria.includes('end')) {
    console.log(`🎯🎯🎯 FOUND LEAVE/END! Calling stopRecording() NOW! 🎯🎯🎯`);
    stopRecording();
  }
...
function startMeetingDetection() {
  ...
  setInterval(() => {
    ...
    if (meetingDetected && !lastState && !isInMeeting) {
      ...
      meetingExitDetected = false;
      startMeetingWithDelay();
    } else if (!meetingDetected && lastState && isInMeeting && !meetingExitDetected) {
      console.log("🛑 MEETING ENDED - CONTAINER DISAPPEARED!");
      meetingExitDetected = true;
      meetingEnded();
    }
...
function setupEndButtonDetection() {
  ...
  // ============================================================
  // SEARCH FOR LEAVE BUTTON IN SHADOW DOM
  // ============================================================
  function attachLeaveButtonListener() {
    console.log("🔍 Searching for Leave button in Shadow DOM...");
    
    const leaveBtn = findInShadow(document.body, 'leave meeting');
    const endBtn = findInShadow(document.body, 'end meeting');
    const targetBtn = leaveBtn || endBtn;
    
    if (targetBtn) {
      console.log("✅ FOUND LEAVE/END BUTTON IN SHADOW DOM!");
      console.log("   Text:", targetBtn.textContent.substring(0, 100));
      console.log("   Tag:", targetBtn.tagName);
      
      // Find the actual clickable button
      const clickableBtn = targetBtn.closest('button') || targetBtn.closest('[role="button"]') || targetBtn;
      
      if (clickableBtn && !clickableBtn.hasAttribute('data-recorder-leave-listener')) {
        clickableBtn.setAttribute('data-recorder-leave-listener', 'true');
        
        clickableBtn.addEventListener('click', () => {
          console.log("🎯🎯🎯 LEAVE BUTTON CLICKED!!! STOPPING RECORDING NOW!!! 🎯🎯🎯");
          stopRecording();
        }, true);
        
        console.log("✅ Click listener attached to Leave button");
      }
    } else {
      console.log("⚠️ Leave button not found in Shadow DOM yet");
    }
  }
  ...
  // Also search periodically in case button appears later
  setInterval(() => {
    const leaveBtn = findInShadow(document.body, 'leave meeting');
    const endBtn = findInShadow(document.body, 'end meeting');
    const targetBtn = leaveBtn || endBtn;
    
    if (targetBtn && !targetBtn.hasAttribute('data-recorder-leave-listener')) {
      console.log("🔍 Re-found Leave button, attaching listener...");
      attachLeaveButtonListener();
    }
  }, 1000);
